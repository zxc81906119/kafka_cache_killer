...
public abstract class BaseMvcTest extends BaseTest {
    ...
    private static String getContentType(String annotationContentType, MockHttpServletRequestBuilder mockHttpServletRequestBuilder) {
    ...
    private static void jsonDataProcess(MockHttpServletRequestBuilder mockHttpServletRequestBuilder, String jsonData) {
        mockHttpServletRequestBuilder.content(jsonData);
    }

    private static void formDataProcess(MockHttpServletRequestBuilder mockHttpServletRequestBuilder, String jsonData) throws JsonProcessingException {
    ...
    @SneakyThrows
    private static MvcTestInfo getMvcInfo(Class<? extends BaseMvcTest> aClass, String methodName) {
        val method = aClass.getDeclaredMethod(methodName);
        return Optional.ofNullable(method.getAnnotation(MvcInfo.class))
                .map((mvcInfo) -> new MvcTestInfo(methodName, aClass.getSimpleName(), mvcInfo))
                .orElse(null);
    }

    @SneakyThrows
    private static boolean isTestClass(Class<? extends BaseMvcTest> thisClass, StackTraceElement stackTraceElement) {
    ...
    public void setContentToBody(
        ...
        if (HttpMethod.GET.equals(httpMethod)) {
            queryDataProcess(mockHttpServletRequestBuilder, jsonData);
        } else {
            switch (contentType) {
    ...
    private void queryDataProcess(MockHttpServletRequestBuilder mockHttpServletRequestBuilder, String jsonData) throws JsonProcessingException {
        val mapForList = OBJECT_MAPPER.readValue(jsonData,
                new TypeReference<Map<String, List<String>>>() {
                });
        mapForList.forEach((key, value) -> {
            if (value != null) {
                mockHttpServletRequestBuilder
                        .queryParam(key, value.toArray(String[]::new));
            }
        });
    }
